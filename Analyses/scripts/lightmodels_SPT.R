...
write_csv(k2, "NMDS_RambspcPSPSFT3.csv")
...
weeknd <- subset(k2, species== "THUPLI")
plot(value ~ variable , data= weeknd)
...
for (i in (path)){
  filename<- paste("output/Species_models_SPT",i,".pdf", sep = "")
  pdf(file= filename)
...
for (i in (path2)){
  filename<- paste("output/Species_models_SPT2",i,".pdf", sep = "")
  pdf(file= filename)
